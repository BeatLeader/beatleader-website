<script>
	...
	if ($configStore.preferences.theme != 'default') {
		...
		setGlobalCSSValue(
			'customizable-color-1',
			$configStore.preferences.bgColor == 'rgba(29, 7, 34, 0.6282)' ? 'rgba(33, 7, 39, 0.7041)' : $configStore.preferences.bgColor
		);
		setGlobalCSSValue(
			'customizable-color-2',
			$configStore.preferences.headerColor == 'rgba(53, 0, 70, 0.2)' ? 'rgba(0, 0, 0, 0.7418)' : $configStore.preferences.headerColor
		);
	}
</script>

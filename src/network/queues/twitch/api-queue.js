...
export default (options = {}) => {
	...
	const getAuthUrl = (state = '', scopes = '') =>
		substituteVars(AUTHORIZATION_URL, {state: encodeURIComponent(state), scopes: encodeURIComponent(scopes)});
	...
	const profile = async (accessToken, login, priority = PRIORITY.FG_LOW, options = {}) =>
		fetchApi(substituteVars(PROFILE_URL, {login: encodeURIComponent(login)}), accessToken, priority, options);

	const videos = async (accessToken, userId, type = 'archive', priority = PRIORITY.FG_LOW, options = {}) =>
		fetchApi(
			substituteVars(VIDEOS_URL, {userId: encodeURIComponent(userId), type: encodeURIComponent(type)}),
			accessToken,
			priority,
			options
		);
	...
	const streams = async (accessToken, userId, priority = PRIORITY.FG_LOW, options = {}) =>
		fetchApi(substituteVars(STREAMS_URL, {userId: encodeURIComponent(userId)}), accessToken, priority, options);

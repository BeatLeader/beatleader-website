...
export default (options = {}) => {
  ...
  const player = async (playerId, signal = null, priority = PRIORITY.FG_LOW) => fetchJson(substituteVars(SS_API_PLAYER_INFO_URL, {playerId}), {signal}, priority);
  const recentScores = async (playerId, page = 1, signal = null, priority = PRIORITY.FG_LOW) => fetchScores(SS_API_RECENT_SCORES_URL, playerId, page, signal, priority);
  const topScores = async (playerId, page = 1, signal = null, priority = PRIORITY.FG_LOW) => fetchScores(SS_API_TOP_SCORES_URL, playerId, page, signal, priority);

...
const process = response => {
	if (!response || !Array.isArray(response.responses) || response.responses.length !== 2 || !Array.isArray(response.responses[0]))
		return [];
	...
	return {
		...
		scores: response.responses[0].map(s => {
			let {accuracy: acc, ap, rank, score, timeSet, playerId, playerName: name, ...rest} = s;

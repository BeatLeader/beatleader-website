...
export default () => {
	...
	const getPlayer = async playerId =>
		resolvePromiseOrWaitForPending(`accSaberPlayer/${playerId}`, () =>
			accSaberPlayersRepository().getAllFromIndex('accsaber-players-playerId', playerId)
		);
	...
	const getPlayerHistory = async playerId =>
		resolvePromiseOrWaitForPending(`accSaberPlayerHistory/${playerId}`, () =>
			accSaberPlayersHistoryRepository().getAllFromIndex('accsaber-players-history-playerId', playerId)
		);
	...
	const shouldRefresh = async (type = 'all', forceUpdate = false) => {
		if (!forceUpdate) {
			...
			if (lastUpdated && lastUpdated > new Date() - REFRESH_INTERVAL) {
				log.debug(
					`Refresh interval for ${type} not yet expired, skipping. Next refresh on ${formatDate(addToDate(REFRESH_INTERVAL, lastUpdated))}`,
					'AccSaberService'
				);
	...
	const fetchScoresPage = async (playerId, page = 1, priority = PRIORITY.FG_LOW, {...options} = {}) => {
		...
		return (
			await resolvePromiseOrWaitForPending(`fetchPlayerScores/${playerId}/${page}`, () =>
				accSaberScoresApiClient.getProcessed({...options, playerId, page, priority})
			)
		).map(s => ({
			...s,
			leaderboard: {
	...
	const refreshCategories = async (forceUpdate = false, priority = queues.PRIORITY.BG_NORMAL, throwErrors = false) => {
		...
		try {
			...
			if (newCategories && newCategories.length) log.debug(`${newCategories.length} new categories found`, 'AccSaberService');
	...
	const updatePlayerHistory = async player => {
		...
		try {
			...
			if (lastUpdated && lastUpdated > new Date() - REFRESH_INTERVAL) {
				log.debug(
					`Refresh interval for player ${player.playerId} history not yet expired, skipping. Next refresh on ${formatDate(
	...
	const refreshAll = async (category = 'overall', forceUpdate = false, priority = queues.PRIORITY.BG_NORMAL, throwErrors = false) => {
		...
		try {
			...
			const allRankings = await Promise.all(dbCategories.all.map(c => c.name).map(async category => refreshRanking(category)));

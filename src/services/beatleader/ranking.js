...
export default () => {
  ...
  const fetchGlobal = async (page = 1, count = 50, filters, priority = PRIORITY.FG_LOW, signal = null) => resolvePromiseOrWaitForPending(`apiClient/ranking/global/${page}`, () => playersGlobalRankingApiClient.getProcessed({page, count, filters, signal, priority}));

  const fetchCountry = async (country, page = 1, count = 50, filters, priority = PRIORITY.FG_LOW, signal = null) => resolvePromiseOrWaitForPending(`pageClient/ranking/${country}/${page}`, () => playersCountryRankingApiClient.getProcessed({country, page, count, filters, signal, priority}));

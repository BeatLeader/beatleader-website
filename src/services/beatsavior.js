...
export default () => {
	...
	const getPlayerScoresWithScoreSaber = async playerId => {
		const [beatSaviorData, playerScores] = await Promise.all([getPlayerScores(playerId), null]);
	...
	const getPlayerScoresPage = async (playerId, serviceParams = {sort: 'date', order: 'desc', page: 1}) => {
		...
		return {
			...
			data: playerScores.slice(startIdx, startIdx + PLAYER_SCORES_PER_PAGE).map(bs => {
	...
	const refresh = async (playerId, force = false, priority = PRIORITY.BG_NORMAL, throwErrors = false) => {
		...
		try {
			...
			const nextUpdate =
				bsPlayerInfo && bsPlayerInfo.lastRefresh ? addToDate(REFRESH_INTERVAL, bsPlayerInfo.lastRefresh) : addToDate(-SECOND);
	...
	const isDataForPlayerAvailable = async playerId =>
		(await beatSaviorRepository().getFromIndex('beat-savior-playerId', playerId)) !== undefined;

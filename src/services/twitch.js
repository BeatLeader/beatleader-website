...
export default () => {
	...
	const fetchProfile = async (login, priority = PRIORITY.FG_LOW, {fullResponse = false, ...options} = {}) => {
		...
		return resolvePromiseOrWaitForPending(`profileApiClient/${login}/${fullResponse}`, () =>
			profileApiClient.getProcessed({...options, accessToken: token.accessToken, login, priority, fullResponse})
		);
	};
	...
	const fetchVideos = async (userId, priority = PRIORITY.FG_LOW, {fullResponse = false, ...options} = {}) => {
		...
		return resolvePromiseOrWaitForPending(`videosApiClient/${userId}/${fullResponse}`, () =>
			videosApiClient.getProcessed({...options, accessToken: token.accessToken, userId, priority, fullResponse})
		);
	};
		...
		try {
			...
			if (!forceUpdate) {
				if (lastUpdated && lastUpdated > new Date() - REFRESH_INTERVAL) {
					log.debug(
						`Refresh interval not yet expired, skipping. Next refresh on ${formatDate(addToDate(REFRESH_INTERVAL, lastUpdated))}`,
						'TwitchService'
					);

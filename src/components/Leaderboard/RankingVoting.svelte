<script>
  ...
  let selectedTypes = []
  const allMapTypes = ['acc', 'tech', 'midspeed', 'speed']
  let mapTypes = ['acc', 'tech', 'midspeed', 'speed']
  let selectedType = '+'
...
<div class="ranking-voting">
  ...
  >
    <div slot="content">
      ...
      <Button
        label="NO"
        type={suitableForRank || suitableForRank == undefined
          ? 'default'
          : 'danger'}
      ...
      <Button
        label="YES"
        type={suitableForRank === false || suitableForRank == undefined
          ? 'default'
          : 'green'}
          ...
          {#each selectedTypes as type, idx}
            <div>
              ...
              <button
                class="remove-type"
                title="Remove"
                on:click={() => remove(type)}><i class="fas fa-xmark" /></button
              >
            </div>
          {/each}
          <Select

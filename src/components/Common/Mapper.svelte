<script>
	...
	import PlayerNameWithFlag from './PlayerNameWithFlag.svelte';
	...
	<div class="mapper">
		...
		{#if !compact}
			<div class="mapper-name">
				<PlayerNameWithFlag player={user} on:click={user ? () => navigateToPlayer(playerId) : null} disablePopover, {hideFlag} />
			</div>
		{/if}
	</div>
</div>

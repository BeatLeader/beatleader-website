...
<span class="flex">
  <input type="number" min={minValue} max={Math.min(to, maxValue)} {step} bind:value={from}
         class:open={open} bind:this={filterEl} on:input={debouncedOnFromChanged}
         title="Use the up/down arrow to quickly change the value"
  />
  <span>to</span>
  <input type="number" min={Math.max(from, minValue)} max={maxValue} {step} bind:value={to}
         class:open={open} on:input={debouncedOnToChanged}
         title="Use the up/down arrow to quickly change the value"
  />
</span>

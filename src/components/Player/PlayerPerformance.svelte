<script>
  ...
  $: beatSavior = songScore?.beatSavior ?? getBeatSaviorCompatibleStats(score)
	...
	$: prevMistakes = improvements && beatSavior?.stats && improvements.timeset?.length && improvements.score
            ...
            <Badge onlyLabel={true} color="white" bgColor="var(--ppColour)">
              <span slot="label">
                <Pp playerId={score.playerId} leaderboardId={leaderboard.leaderboardId}
                    pp="{score.pp}" bonusPp={score.bonusPp} weighted={score.ppWeighted} {improvements}
                ...
                <span slot="label"
                  ...
                  {#if beatSavior.stats.miss || beatSavior.stats.bombHit || beatSavior.stats.wallHit}
                    ...
                    <Value
											...
											prevValue={prevMistakes} forcePrev={Number.isFinite(prevMistakes)}
											inline={true} digits={0}
										>
											<small slot="prev">
												{#if Number.isFinite(prevMistakes)}
														(vs
														{#if prevMistakes}
															<i class="fas fa-times"></i> {formatNumber(prevMistakes, 0)}
														{:else}
															FC
														{/if}
														)
                  ...
                  {:else}
										<span class="fc">
											FC
											{#if Number.isFinite(prevMistakes)}
												<small title={`Missed notes: ${prevMissedNotes}, Bad cuts: ${prevBadCuts}, Bomb hit: ${prevBombHit}, Wall hit: ${prevWallsHit}`}>
													(vs
													{#if prevMistakes}
														<i class="fas fa-times"></i> {formatNumber(prevMistakes, 0)}
													{:else}
  ...
  {#if (showDetails || (configStore && opt($configStore, 'scoreComparison.method') === 'in-place')) && myScore}
    ...
    <div class="player-performance-badges">
        ...
        <span class="pp with-badge compare">
            <Badge onlyLabel={true} color="white" bgColor="var(--ppColour)">
              <span slot="label">
                <Pp playerId={myScore?.playerId} leaderboardId={leaderboard.leaderboardId}
										pp="{myScore?.score.pp}" withZeroSuffix={true} inline={false}
										color="white"
								/>
              </span>
            </Badge>

		...
		return categories && playerInfo && categories.length && playerInfo.length
			? categories
					...
					.map(c => ({
						...c,
						prevPlayerInfo:
							c.playerInfo && playerHistoryGain?.categories?.[c.name]
								? {
										...c.playerInfo,
			...
			<div>
				<div>
					...
					{#each playerInfoByCategory as category (category.name)}
						<Badge
							...
							value={category.playerInfo.rank}
							prefix="#"
							prevValue={category?.prevPlayerInfo?.rank}
							prevLabel={category?.prevPlayerInfo?.rank && Number.isFinite(category?.prevPlayerInfo?.gainDaysAgo)
								? formatDateRelative(addToDate(-category?.prevPlayerInfo?.gainDaysAgo * DAY))
								: null}
				...
				<div>
					{#each playerInfoByCategory as category (category.name)}
						<Badge
							label={category.displayName ?? category.name}
							value={category.playerInfo.ap}
							prevValue={category?.prevPlayerInfo?.ap}
							prevLabel={category?.prevPlayerInfo?.ap && Number.isFinite(category?.prevPlayerInfo?.gainDaysAgo)
								? formatDateRelative(addToDate(-category?.prevPlayerInfo?.gainDaysAgo * DAY))
								: null}
							suffix=" AP"
							fluid={true}
			...
			<div>
				<div>
					{#each playerInfoByCategory as category (category.name)}
						<Badge
							label={category.displayName ?? category.name}
							value={category.playerInfo.averageAcc * 100}
							suffix="%"
							prevValue={category?.prevPlayerInfo?.averageAcc ? category?.prevPlayerInfo?.averageAcc * 100 : null}
							prevLabel={category?.prevPlayerInfo?.averageAcc && Number.isFinite(category?.prevPlayerInfo?.gainDaysAgo)
								? formatDateRelative(addToDate(-category?.prevPlayerInfo?.gainDaysAgo * DAY))
								: null}
							fluid={true}
							inline={true}
				...
				<div>
					{#each playerInfoByCategory as category (category.name)}
						<Badge
							label={category.displayName ?? category.name}
							value={category.playerInfo.rankedPlays}
							suffix=" play(s)"
							prevValue={category?.prevPlayerInfo?.rankedPlays}
							prevLabel={category?.prevPlayerInfo?.rankedPlays && Number.isFinite(category?.prevPlayerInfo?.gainDaysAgo)
								? formatDateRelative(addToDate(-category?.prevPlayerInfo?.gainDaysAgo * DAY))
								: null}
							prevSuffix=" "
							digits={0}

	...
	function calculateStats(scores) {
			...
			(stats, s) => {
				...
				['left', 'right'].forEach(key => {
					...
					if (Number.isFinite(s?.trackers?.accuracyTracker?.[accVar])) {
						...
						stats[timeDependenceVar] += Number.isFinite(s?.trackers?.accuracyTracker?.[timeDependenceVar])
							? s?.trackers?.accuracyTracker?.[timeDependenceVar]
							: 0;
		...
		if (stats.total) {
			...
			['left', 'right'].forEach(key => {
				...
				[`acc${keyCapitalized}`, `${key}Miss`, `${key}BadCuts`, `${key}TimeDependence`, `${key}Preswing`, `${key}Postswing`].forEach(
					keyVar => {
				...
				if (stats[cutVar] && Array.isArray(stats[cutVar])) stats[cutVar].forEach((v, idx) => (stats[cutVar][idx] = v / stats[totalVar]));
			});

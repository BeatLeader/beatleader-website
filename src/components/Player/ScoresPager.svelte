<script>
	...
	async function refreshAllPlayerServiceScores(playerId, service, serviceParams) {
		...
		if (service === 'beatleader') playerScores = null;
		else playerScores = (await serviceObj.getPlayerScores(playerId)).filter(playerScoresHistogram.filter).sort(playerScoresHistogram.sort);
	}

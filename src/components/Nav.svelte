<script>
	...
	var testOptions = [
		{
			name: 'Stage',
			baseLink: 'https://stage.beatleader.net',
		},
		{
			name: 'Stage 2',
			baseLink: 'https://stage2.beatleader.net',
		},
		{
			name: 'Stage 3',
			baseLink: 'https://stage3.beatleader.net',
		},
	];
...
<nav class="ssr-page-container">
	...
	<div
		class="hovermenu"
		on:mouseover={() => (testMenuShown = true)}
		on:focus={() => (testMenuShown = true)}
		on:mouseleave={() => (testMenuShown = false)}>
		...
		<Dropdown items={testOptions} shown={testMenuShown}>
			<svelte:fragment slot="row" let:item>
				<a style="display: block; width: 100%" href={currenturl.replace('https://www.beatleader.xyz', item.baseLink)}> {item.name} </a>
			</svelte:fragment>
		</Dropdown>
	</div>
